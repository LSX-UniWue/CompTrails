...
class SyntheticDataset:
    def __init__(self, args: dict) -> None:
        ...
        self.transition_counts_per_row = args['transition_counts_per_row']
        ...
        if args['data_generation'] == 'random':
            ...
            for i, (transition, hypothesis) in enumerate(zip(self.transitions, self.hypotheses)):
                print(f"Network {i}: Transitions-{transition.sum()}, entropy: {entropy(transition).mean()} Hypothesis-{hypothesis.sum()}, Entropy: {entropy(hypothesis).mean()}")
            ...
            sys.exit(1)
        self.transitions_per_edges = [[x.sum()/np.count_nonzero(x) for x in m] for m in self.transitions]
    ...
    def create_transitions(self, i: int) -> np.ndarray:
        ...
        transition_probas = ((1 - self.mixtures[i]) * new_random + self.mixtures[i] * self.hypotheses[i])
        transitions = []
        for row_proba in transition_probas:
            samples = np.random.choice(self.sizes[i], size=self.transition_counts_per_row[i], p=row_proba)
            reduce_by_key = np.array([len(np.where(samples == x)[0]) for x in range(self.sizes[i])])
            transitions.append(reduce_by_key)
        return np.array(transitions)
        # return (transition_probas * 1000).astype(int)
    ...
    def sample_transitions(transitions: list, size: int, normalise: bool = False) -> list:
        ...
        for row in range(transitions[0].shape[0]):
            sample_size = int(min([x[row].sum() for x in transitions]))
    ...
    def calculate_js_divergence(self) -> list:
        divergence = []
        if self.is_sampling:
            for _ in tqdm(range(self.runs), desc="Collecting divergence. "):
                curr_divergence = []
                iter_trans, iter_hyp = [], []
                for transitions, hypothesis in zip(self.transitions, self.hypotheses):
                    curr_transitions, curr_hypothesis = self.sample_states(transitions=transitions, hypothesis=hypothesis, size=self.sample_size)
                    iter_trans.append(curr_transitions)
                    iter_hyp.append(curr_hypothesis)
                iter_trans_edge_sampled = self.sample_transitions(iter_trans, size=self.sample_size, normalise=True)
                for transitions, hypothesis in zip(iter_trans_edge_sampled, iter_hyp):
...
def print_divergence(divergence: list, x_ticks: list, save_path: str, is_sampled: bool) -> None:
    ...
    if is_sampled:
        ...
        plt.ylim((min(divergence) - 0.1), (max(divergence) + 0.1))
...
different_node_settings = {
    'name': 'node',
    'sizes': [10, 100, 1000], 
    'transition_counts_per_row': [10, 100, 1000],  # for random dataset
    'transition_counts': [5, 25, 125], 
    'walk_lengths': [5, 5, 5], 
    'walks_per_node': 5, 
...
different_edge_settings = {
    'name': 'edge',
    'sizes': [100, 100, 100], 
    'transition_counts_per_row': [10000, 100000, 1000000],  # for random dataset
    'transition_counts': [25, 25, 25], 
...
if __name__ == '__main__':
    settings = different_distr_settings
    if LOAD: 
        ...
        sampled_dataset = SyntheticDataset(args={
            ...
            'transition_counts_per_row': settings['transition_counts_per_row'],
        ...
        # print(f"Transitions per Edges: {sampled_dataset.get_transition_count_per_edges()}/{unsampled_dataset.get_transition_count_per_edges()}")
